...
def createTrainData(video_path, character_name, each_video_data_num = 1000, path = None):
    dir_path = pathlib.Path(sys.argv[-1]) if path is None else path
    #カスケード分類器の特徴量を取得する
    cascade_path = resource_path(str(dir_path) + "/config/lbpcascade_animeface.xml")
